$(document).ready(function() {
  ...
  var RMPRegex = /^(http:\/\/|https:\/\/)?(www.)?ratemyprofessors.com\/ShowRatings.jsp\?(.*)?tid\=([0-9]+)(.*)?$/
  ...
  $loadProfButton.on('click', function() {
    profRMPId = null; // Reset everytime button is clicked

    var profUrl = $loadProfUrl.val()
    var RMPUrlMatch = profUrl.match(RMPRegex);

    if(RMPUrlMatch !== null) {
      profRMPId = RMPUrlMatch[4];
    }

    if(profRMPId == null || isNaN(profRMPId)) {
      profRMPId = null
      alert('The url must start with:\nhttps://www.ratemyprofessors.com/ShowRatings.jsp?tid=')
        ...
        $.ajax({
          ...
          if(response == 'NULL') {
            ...
            $profName.html(titleSections[0])
            $uniName.html(titleSections[1])
          }
        });
    }
  })
  ...
  var $profRMPIdInput = $('#profRMPIdInput')
